...
function test()
	...
	for ws in sheet
		...
		set_row!(ws, 0, height=20, fmt=bold)
		# Add header text
	...
	for r in 1:size(data, 1)
		...
		if data[r,1] != "East"
			set_row!(sheet[2], r, options=Dict("hidden"=>"1"))
		end
	...
	for r in 1:size(data, 1)
		...
		if data[r,1] != "East" && data[r,1] != "South"
			set_row!(sheet[3], r, options=Dict("hidden"=>true))
		end
	...
	for r in 1:size(data, 1)
		...
		if !(data[r,1] == "East" && data[r,3] > 3000 && data[r,3] < 8000)
			set_row!(sheet[4], r, options=Dict("hidden"=>true))
		end
	...
	for r in 1:size(data, 1)
		...
		if data[r,1] != ""
			...
			set_row!(sheet[5], r, options=Dict("hidden"=>true))
		end
	...
	for r in 1:size(data, 1)
		...
		if data[r,1] == ""
			# Row matches the filter, no further action required.
			set_row!(sheet[6], r, options=Dict("hidden"=>true))
		end
		write_row!(sheet[6], r, 0, data[r,:])
	end

	close(wb)
	isfile("autofilter.xlsx")
end
